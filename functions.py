...
#parameter import from yaml file
...
def show_instruction(file_path):
    text_content = "Brak pliku instrukcji. Naciśnij spację, aby kontynuować."
    if os.path.exists(file_path):
        with open(file_path, "r", encoding="utf-8") as f:
            text_content = f.read()

    instr_stim = visual.TextStim(win, text=text_content, color='white', height=24, wrapWidth=800)
    instr_stim.draw()
    win.flip()
...
def check_correct(stim, trial_type, focus_time,
    ...
    if keys:
        ...
        rt = keys[0].rt * 1000.0  # konwersja na ms

        correct_key = answers_color[stim] if trial_type == "color" else answers_shape[stim]
        correct_answer = (response_key == correct_key)
    else:
        answered = False
...
def practice_run_single(n, trial_type):
    ...
    for i in range(n):
        ...
        res = check_correct(stim=stim, trial_type=trial_type, focus_time=focus_time)
...
def testing_run_single(n, trial_type, results):
    ...
    for i in range(n):
        ...
        save_data(result_list=results, block_name=f"Single_{trial_type}", stim=stim, trial_type=trial_type,
                  is_switch=False)
...
def trial(single_training_n=None, single_trial_n=None, mixed_training_n=None, mixed_trial_n=None):
    ...
    # GŁÓWNA INSTRUKCJA
    show_instruction(os.path.join("instructions", "instrukcja_glowna.txt"))

    # TRENING KOLOR
    show_instruction(os.path.join("instructions", "trening_kolor.txt"))
    practice_run_single(single_training_n, "color")

    # PRÓBA KOLOR
    show_instruction(os.path.join("instructions", "test_kolor.txt"))
    testing_run_single(single_trial_n, "color", results=results)

    # TRENING KSZTAŁT
    show_instruction(os.path.join("instructions", "trening_ksztalt.txt"))
    practice_run_single(single_training_n, "shape")

    # PRÓBA KSZTAŁT
    show_instruction(os.path.join("instructions", "test_ksztalt.txt"))
    testing_run_single(single_trial_n, "shape", results=results)

    # TRENING MIESZANE
    show_instruction(os.path.join("instructions", "trening_mieszane.txt"))
    practice_run_mixed(mixed_training_n)

    # PRÓBA MIESZANE
    show_instruction(os.path.join("instructions", "test_mieszane.txt"))
    testing_run_mixed(mixed_trial_n, results=results)

    # ZAPIS DANYCH
    save_tofile(results, "file_name.csv", "results")
